...
async function findActiveUserByEmployeeId(employeeId) {
  const result = await query(
    ...
    SELECT
      ...
      u.SectionId,
      s.SectionName,
      COALESCE((SELECT TOP 1 atw.WorkspaceId FROM dbo.UserAssignments ua JOIN dbo.AssignmentTypeWorkspaces atw ON atw.AssignmentTypeId=ua.AssignmentTypeId AND atw.IsActive=1 JOIN dbo.Workspaces aw ON aw.WorkspaceId=atw.WorkspaceId AND aw.IsActive=1 WHERE ua.UserId=u.UserId AND ua.IsActive=1 AND ua.IsPrimary=1),(SELECT WorkspaceId FROM dbo.Workspaces WHERE WorkspaceId=u.DefaultWorkspaceId AND IsActive=1),(SELECT TOP 1 wr.WorkspaceId FROM dbo.WorkspaceRoles wr JOIN dbo.Workspaces rw ON rw.WorkspaceId=wr.WorkspaceId AND rw.IsActive=1 WHERE wr.RoleId=u.RoleId AND wr.IsDefault=1 ORDER BY rw.SortOrder)) DefaultWorkspaceId,
      (SELECT WorkspaceName FROM dbo.Workspaces WHERE IsActive=1 AND WorkspaceId=COALESCE((SELECT TOP 1 atw.WorkspaceId FROM dbo.UserAssignments ua JOIN dbo.AssignmentTypeWorkspaces atw ON atw.AssignmentTypeId=ua.AssignmentTypeId AND atw.IsActive=1 JOIN dbo.Workspaces aw ON aw.WorkspaceId=atw.WorkspaceId AND aw.IsActive=1 WHERE ua.UserId=u.UserId AND ua.IsActive=1 AND ua.IsPrimary=1),(SELECT WorkspaceId FROM dbo.Workspaces WHERE WorkspaceId=u.DefaultWorkspaceId AND IsActive=1),(SELECT TOP 1 wr.WorkspaceId FROM dbo.WorkspaceRoles wr JOIN dbo.Workspaces rw ON rw.WorkspaceId=wr.WorkspaceId AND rw.IsActive=1 WHERE wr.RoleId=u.RoleId AND wr.IsDefault=1 ORDER BY rw.SortOrder))) DefaultWorkspaceName,
      COALESCE((SELECT TOP 1 w.DefaultRoute FROM dbo.UserAssignments ua JOIN dbo.AssignmentTypeWorkspaces atw ON atw.AssignmentTypeId=ua.AssignmentTypeId AND atw.IsActive=1 JOIN dbo.Workspaces w ON w.WorkspaceId=atw.WorkspaceId AND w.IsActive=1 WHERE ua.UserId=u.UserId AND ua.IsActive=1 AND ua.IsPrimary=1),(SELECT DefaultRoute FROM dbo.Workspaces WHERE WorkspaceId=u.DefaultWorkspaceId AND IsActive=1),(SELECT TOP 1 w.DefaultRoute FROM dbo.WorkspaceRoles wr JOIN dbo.Workspaces w ON w.WorkspaceId=wr.WorkspaceId AND w.IsActive=1 WHERE wr.RoleId=u.RoleId ORDER BY wr.IsDefault DESC,w.SortOrder)) AS DefaultWorkspaceRoute,
      u.LegacyRole,
