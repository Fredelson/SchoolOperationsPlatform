...
async function getSidebarMenusForUser(userId) {
  const result = await executeQuery(
    ...
    SELECT
        ...
        ISNULL(wm.IsEnabled, 0) AS IsEnabled,
        ISNULL(wmodule.IsEnabled, 0) AS ModuleIsEnabled,
    ...
    LEFT JOIN dbo.WorkspaceModules wmodule ON wmodule.ModuleId = module.ModuleId AND wmodule.WorkspaceId = @NavigationWorkspaceId
    ...
    AND wmodule.WorkspaceModuleId IS NOT NULL
    AND wmodule.IsVisible = 1
    AND (
      (
        @IsSuperAdmin=1
...
async function getRuntimeControlsForUser(userId) {
  const result=await executeQuery(`
    DECLARE @WorkspaceId int=COALESCE((SELECT TOP 1 atw.WorkspaceId FROM dbo.UserAssignments ua JOIN dbo.AssignmentTypes at ON at.AssignmentTypeId=ua.AssignmentTypeId AND at.IsActive=1 JOIN dbo.AssignmentTypeWorkspaces atw ON atw.AssignmentTypeId=ua.AssignmentTypeId AND atw.IsActive=1 JOIN dbo.Workspaces aw ON aw.WorkspaceId=atw.WorkspaceId AND aw.IsActive=1 WHERE ua.UserId=@UserId AND ua.IsActive=1 AND ua.IsPrimary=1 AND (ua.StartDate IS NULL OR ua.StartDate<=CAST(GETDATE() AS date)) AND (ua.EndDate IS NULL OR ua.EndDate>=CAST(GETDATE() AS date)) ORDER BY ua.UpdatedAt DESC,ua.CreatedAt DESC,ua.UserAssignmentId DESC),(SELECT TOP 1 wr.WorkspaceId FROM dbo.Users u JOIN dbo.WorkspaceRoles wr ON wr.RoleId=u.RoleId JOIN dbo.Workspaces rw ON rw.WorkspaceId=wr.WorkspaceId AND rw.IsActive=1 WHERE u.UserId=@UserId ORDER BY wr.IsDefault DESC,rw.SortOrder));
    IF EXISTS(SELECT 1 FROM dbo.Users u JOIN dbo.Roles r ON r.RoleId=u.RoleId WHERE u.UserId=@UserId AND r.RoleKey IN ('SuperAdmin','PlatformAdmin'))
    BEGIN
      SELECT b.ButtonId,b.ButtonKey,b.ButtonName,b.ModuleId,wb.IsEnabled,wb.SortOrder FROM dbo.WorkspaceButtons wb JOIN dbo.Buttons b ON b.ButtonId=wb.ButtonId ORDER BY wb.SortOrder,b.ButtonName;
      SELECT w.WidgetId,w.WidgetKey,w.WidgetName,w.WidgetType,w.DataSourceKey,w.ModuleId,ww.SortOrder FROM dbo.WorkspaceWidgets ww JOIN dbo.Widgets w ON w.WidgetId=ww.WidgetId ORDER BY ww.SortOrder,w.WidgetName;
    END
    ELSE
    BEGIN
      SELECT b.ButtonId,b.ButtonKey,b.ButtonName,b.ModuleId,wb.IsEnabled,wb.SortOrder FROM dbo.WorkspaceButtons wb JOIN dbo.Buttons b ON b.ButtonId=wb.ButtonId WHERE wb.WorkspaceId=@WorkspaceId AND wb.IsVisible=1 AND wb.IsEnabled=1 ORDER BY wb.SortOrder,b.ButtonName;
      SELECT w.WidgetId,w.WidgetKey,w.WidgetName,w.WidgetType,w.DataSourceKey,w.ModuleId,ww.SortOrder FROM dbo.WorkspaceWidgets ww JOIN dbo.Widgets w ON w.WidgetId=ww.WidgetId WHERE ww.WorkspaceId=@WorkspaceId AND ww.IsVisible=1 AND ww.IsEnabled=1 ORDER BY ww.SortOrder,w.WidgetName;
    END
