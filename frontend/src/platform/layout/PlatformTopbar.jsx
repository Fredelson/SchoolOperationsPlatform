...
import {API_MUTATION_EVENT} from "../../services/api";
import {getNotificationFeedService} from "../../modules/notifications/services/notificationFeedService";
...
import useAppNotification from "../ui/feedback/useAppNotification";
...
export default function PlatformTopbar({height=78,onMenuClick}){
  const theme=useTheme(),navigate=useNavigate(),location=useLocation(),{user,logout}=useAuth(),notification=useAppNotification();
  ...
  const role=normalized(effectiveUser?.roleKey||effectiveUser?.role),admin=role==="superadmin"||role==="platformadmin",canNotify=Boolean(effectiveUser?.UserId||effectiveUser?.id);
  const notificationUserId=effectiveUser?.UserId||effectiveUser?.id;
  const notificationReadStorageKey=notificationUserId?`topbarNotificationsReadAt:${notificationUserId}`:null;
  const[anchor,setAnchor]=useState(null),[panel,setPanel]=useState(null),[activities,setActivities]=useState([]),[readAt,setReadAt]=useState(null),[markingRead,setMarkingRead]=useState(false),[dialog,setDialog]=useState(null);
  const refreshActivities=useCallback(async()=>{
    if(!canNotify)return;
    try{
      const feed=await getNotificationFeedService({limit:20});
      setActivities(feed);
    }catch{
  ...
  useEffect(()=>{
    ...
    let mounted=true;
    Promise.all([
      getNotificationFeedService({limit:20}).catch(()=>[]),
      getNotificationReadAtService().then(r=>r.readAt||null).catch(()=>null)
    ]).then(([logs, serverReadAt])=>{
      if(!mounted)return;
      setActivities(logs);
      const persistedReadAt = serverReadAt || (notificationReadStorageKey?localStorage.getItem(notificationReadStorageKey):null) || "";
      setReadAt(persistedReadAt);
    }).catch(()=>{if(mounted){setActivities([]);}});
    return ()=>{mounted=false};
  },[canNotify,notificationReadStorageKey]);
  useEffect(()=>{
    if(!canNotify)return;
    const refreshWhenVisible=()=>{
      if(document.visibilityState==="visible")refreshActivities();
    };
    window.addEventListener(API_MUTATION_EVENT,refreshActivities);
    window.addEventListener("focus",refreshActivities);
    document.addEventListener("visibilitychange",refreshWhenVisible);
    const intervalId=window.setInterval(refreshWhenVisible,30000);
    return ()=>{
      window.removeEventListener(API_MUTATION_EVENT,refreshActivities);
      window.removeEventListener("focus",refreshActivities);
      document.removeEventListener("visibilitychange",refreshWhenVisible);
      window.clearInterval(intervalId);
    };
  },[canNotify,refreshActivities]);
  const unreadActivities=useMemo(()=>activities.filter(x=>!readAt||new Date(x.CreatedAt)>new Date(readAt)),[activities,readAt]);
  const unread=unreadActivities.length;
  const openMenu=(event,type)=>{setAnchor(event.currentTarget);setPanel(type);if(type==="notifications")refreshActivities()},close=()=>{setAnchor(null);setPanel(null)};
  const markRead=async()=>{
    if(!unread||markingRead)return;
    setMarkingRead(true);
    try{
      const result=await markAllNotificationsAsReadService();
      if(!result?.readAt)throw new Error("Notification read state was not returned.");
      if(notificationReadStorageKey)localStorage.setItem(notificationReadStorageKey,result.readAt);
      setReadAt(result.readAt);
    }catch(error){
      notification.showError(error.response?.data?.message||"Could not mark notifications as read.");
    }finally{
  ...
  return <Box sx={{height,px:{xs:2,md:4},display:"flex",alignItems:"center",justifyContent:"space-between",gap:2,color:text,background:topbar,position:"fixed",inset:"0 0 auto 0",zIndex:1300,borderBottom:`1px solid ${alpha(text,.15)}`}}>
    ...
    <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={close} PaperProps={{sx:{width:panel==="user"?220:420,maxWidth:"95vw"}}}>
      {panel==="notifications"&&<Box><Stack direction="row" sx={{justifyContent:"space-between",alignItems:"center",px:2,py:1}}><Typography fontWeight={800}>Notifications</Typography><Button size="small" onClick={markRead} disabled={!unread||markingRead}>{markingRead?"Marking...":"Mark all as read"}</Button></Stack><Divider/>{unreadActivities.length?unreadActivities.map(x=><MenuItem key={x.ActivityTimelineId} onClick={close} sx={{whiteSpace:"normal",alignItems:"flex-start"}}><ListItemText primary={`${x.ActivityTitle||x.ActivityType||"Activity"}${x.ModuleName?` · ${x.ModuleName}`:""}`} secondary={`${x.ActivityDescription||"Module activity"} · ${x.PerformedByName||"System"} · ${new Date(x.CreatedAt).toLocaleString()}`}/></MenuItem>):<MenuItem disabled>No unread notifications</MenuItem>}</Box>}
