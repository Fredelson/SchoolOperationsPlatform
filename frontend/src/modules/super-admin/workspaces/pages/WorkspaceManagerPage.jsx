...
export default function WorkspaceManagerPage() {
  ...
  const toggleAssignment=(index)=>setConfig(previous=>({...previous,modules:previous.modules.map((item,i)=>i===index?{...item,IsAssigned:!item.IsAssigned}:item)}));
  const saveModules=async()=>{setLoading(true);setError("");try{const items=config.modules.filter(x=>x.IsAssigned).map(x=>({id:x.ModuleId,isVisible:true,isEnabled:x.IsEnabled!==false,sortOrder:x.SortOrder}));setConfig(await saveWorkspaceAssignments(workspaceId,"modules",items));}catch(e){setError(e.response?.data?.message||e.message);}finally{setLoading(false);}};
  return <Stack spacing={2}>
    ...
    {loading?<CircularProgress/>:<Paper sx={{p:3}}>
      ...
      {tab===1&&<Button sx={{mb:2}} variant="contained" onClick={saveModules}>Save module assignments</Button>}
      ...
      {Array.isArray(rows)&&tab!==8&&<Stack spacing={1}>{rows.length?rows.map((row,index)=><Paper variant="outlined" sx={{p:1.5}} key={row.ModuleId||row.MenuId||row.ButtonId||row.WidgetId||row.DashboardId||row.RoleId||index}>{tab===1&&<Checkbox checked={Boolean(row.IsAssigned)} onChange={()=>toggleAssignment(index)}/>}<Typography component="span">{row.ModuleName||row.MenuName||row.ButtonName||row.WidgetName||row.DashboardName||row.RoleName}</Typography><Typography variant="caption" display="block" color="text.secondary">{row.BaseRoute||row.Route||row.ModuleKey||row.RoleKey||"Configured record"}</Typography></Paper>):<Typography color="text.secondary">No supported records assigned.</Typography>}</Stack>}
    </Paper>}
  </Stack>;
}
