...
export default function WorkspacePreviewPage() {
  ...
  return <Stack spacing={2}>
    ...
    <Stack direction={{xs:"column",md:"row"}} spacing={2} alignItems="flex-start">
      ...
      <Paper sx={{p:2,flex:1}}><Typography variant="h6">Effective permissions</Typography><Divider sx={{my:1}}/><Stack direction="row" flexWrap="wrap" gap={1}>{data.permissions?.allowedPermissionKeys?.map(key=><Typography key={key} variant="caption" sx={{border:1,borderColor:"divider",borderRadius:1,p:.75}}>{key}</Typography>)}</Stack></Paper>
    </Stack>
  </Stack>;
}
