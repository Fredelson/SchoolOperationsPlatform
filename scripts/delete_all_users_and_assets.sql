...
-- User & Asset Data Reset Script
-- ============================================================
--
-- WARNING:
-- This script DELETES ALL transactional/activity data,
-- all IT assets, and all users.
--
...
DELETE FROM dbo.PrintingRequests WHERE 1 = 1;

PRINT N'--- Deleting all users...';
...
DBCC CHECKIDENT ('dbo.PrintingRequests', RESEED, 0);
DBCC CHECKIDENT ('dbo.PrintingWorkflowEvents', RESEED, 0);
DBCC CHECKIDENT ('dbo.PhotocopyRequests', RESEED, 0);

COMMIT TRANSACTION;

PRINT N'--- Data reset complete. Reference/master data preserved. ---';
